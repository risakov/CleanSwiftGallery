...
protocol AuthUseCase {
    ...
    func logIn(_ login: String, _ password: String) -> Completable
...
class AuthUseCaseImp: AuthUseCase {
    ...
    func logIn(_ login: String, _ password: String) -> Completable {
        return authApi.auth(login, password)
                ...
                .flatMapCompletable(checkUserRole())
    }

    private func checkUserRole() -> (UserEntity) -> Completable {
        return { user in
            guard user.roles.count == 1 && user.roles.first == "ROLE_USER" else {
                return .error(LoginError.notAvailableRole)
            }
